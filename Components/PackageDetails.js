...
const PackageDetails = ({ navigation, route }) => {
  ...
  const [detailsPackage, setDetails] = useState(
    route.params.details.split(",")
  );
  ...
  const handleCheck = (extra) => {
    ...
    extraPrice == 0
      ? setTotalAmount(totalAmount - extra.price)
      : setTotalAmount(totalAmount + extraPrice);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <View style={[styles.card, styles.shadowProp]}>
          ...
          <View style={{ flexDirection: "row" }}>
            <View style={{ width: 220 }}>
              <Text style={{ margin: 3, paddingTop: 10, fontSize: 18 }}>
                {route.params.name}{" "}
              </Text>
              {route.params.cup ? (
                <Text style={{ margin: 6, fontSize: 18 }}>
                  {route.params.cup} Cup
                </Text>
              ) : null}
            </View>
            <View
        ...
        <View
          style={{ marginTop: 10, width: 370, alignSelf: "center", padding: 5 }}
        >
          <Text style={{ fontWeight: "bold", margin: 6, fontSize: 20 }}>
            Catering Menu Include
          </Text>
          {detailsPackage.map((x, i) => {
            return (
              <Text
                key={i}
                style={{ paddingLeft: 10, margin: 3, fontSize: 18 }}
              >
                {x}
              </Text>
            );
